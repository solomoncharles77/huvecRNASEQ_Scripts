...
# Load data
gwas <- data.frame(fread("assocRes/huvevRefSeqAEIGWAS.mlma"))
...
png("assocRes/manPlots/huvevRefSeqAEIGWAS_manhattan_plot.png", width=1200, height=600)
manhattan(gwas, main="huvevRefSeqAEIGWAS",
          suggestiveline = -log10(5e-05))
dev.off()
png("assocRes/qqPlots/huvevRefSeqAEIGWAS_qqplot.png", width=400, height=300) 
qq <- qq(gwas$P)
abline(h = -log10(5e-8), col = "red", lty = 2)
dev.off()  
fwrite(gwas, "resFiles/huvevRefSeqAEIGWAS_GWAS_Summary_Statistics.txt.gz")
...
write.table(suggestive_hits, "assocRes/huvevRefSeqAEIGWAS_suggestiveHits.txt", row.names = F, quote = F, )
clumpHits <- read.table("assocRes/huvevRefSeqAEIGWAS_clumped_suggestive_loci.clumped", header = T)
...
write.csv(gwasTop, "assocRes/huvevRefSeqAEIGWAS_gwasTop.csv", row.names = F )
...
write.csv(annotGWAS2, "resFiles/huvevRefSeqAEIGWAS_mappedGenes.csv", row.names = F)
write.table(data.frame(unique(annotGWAS2$comGeneID)), "resFiles/huvevRefSeqAEI_nearestGenesList.txt",
            row.names = F, col.names = F, quote = F)
write.table(data.frame(unique(annotGWAS2$comTxID)), "resFiles/huvevRefSeqAEI_nearestTranscriptList.txt",
            row.names = F, col.names = F, quote = F)

############################################################################
annotGWAS2 <- read.csv("resFiles/huvevRefSeqAEIGWAS_mappedGenes.csv")
...
png("assocRes/manPlots/huvevRefSeqAEI_manhattanPlot.png", width=1200, height=600)
