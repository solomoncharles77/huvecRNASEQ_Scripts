...
# Load data
gwas <- data.frame(fread("assocRes/huvevMMSitesAEIGWAS.mlma"))
...
png("assocRes/manPlots/huvevMMSitesAEIGWAS_manhattan_plot.png", width=1200, height=600)
manhattan(gwas, main="huvevMMSitesAEIGWAS",
          suggestiveline = -log10(5e-05))
dev.off()
png("assocRes/qqPlots/huvevMMSitesAEIGWAS_qqplot.png", width=400, height=300) 
qq <- qq(gwas$P)
abline(h = -log10(5e-8), col = "red", lty = 2)
dev.off()  
fwrite(gwas, "resFiles/huvevMMSitesAEIGWAS_GWAS_Summary_Statistics.txt.gz")
...
write.table(suggestive_hits, "assocRes/huvevMMSitesAEIGWAS_suggestiveHits.txt", row.names = F, quote = F, )
clumpHits <- read.table("assocRes/huvevMMSitesAEIGWAS_clumped_suggestive_loci.clumped", header = T)
...
write.csv(gwasTop, "assocRes/huvevMMSitesAEIGWAS_gwasTop.csv", row.names = F )
...
write.csv(annotGWAS2, "resFiles/huvevMMSitesAEIGWAS_mappedGenes.csv", row.names = F)
write.table(data.frame(unique(annotGWAS2$comGeneID)), "resFiles/huvevMMSitesAEI_nearestGenesList.txt",
            row.names = F, col.names = F, quote = F)
write.table(data.frame(unique(annotGWAS2$comTxID)), "resFiles/huvevMMSitesAEI_nearestTranscriptList.txt",
            row.names = F, col.names = F, quote = F)

############################################################################
annotGWAS2 <- read.csv("resFiles/huvevMMSitesAEIGWAS_mappedGenes.csv")
...
png("assocRes/manPlots/huvevMMSitesAEI_manhattanPlot.png", width=1200, height=600)
