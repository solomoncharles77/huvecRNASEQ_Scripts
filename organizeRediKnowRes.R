...
cID <- names(freqList)
write.table(data.frame(cID), "rnaEditing_scripts/rediKnownComplete.txt", row.names = F, col.names = F, quote = F)
...
fwrite(freqDF, "phenoFiles/rawRediKnown.txt.gz", row.names = F)
...
freqDF <- data.frame(fread("phenoFiles/rawRediKnown.txt.gz"))
colnames(freqDF) <-  sub("A", "S", colnames(freqDF))
...
fwrite(freqDF1, "phenoFiles/filteredRediKnown.txt.gz", row.names = F)
