...
# Load data
gwas <- data.frame(fread("assocRes/huvevRandomAEIGWAS.mlma"))
...
png("assocRes/manPlots/huvevRandomAEIGWAS_manhattan_plot.png", width=1200, height=600)
manhattan(gwas, main="huvevRandomAEIGWAS",
          suggestiveline = -log10(5e-05))
dev.off()
png("assocRes/qqPlots/huvevRandomAEIGWAS_qqplot.png", width=400, height=300) 
qq <- qq(gwas$P)
abline(h = -log10(5e-8), col = "red", lty = 2)
dev.off()  
fwrite(gwas, "resFiles/huvevRandomAEIGWAS_GWAS_Summary_Statistics.txt.gz")
...
write.table(suggestive_hits, "assocRes/huvevRandomAEIGWAS_suggestiveHits.txt", row.names = F, quote = F, )
clumpHits <- read.table("assocRes/huvevRandomAEIGWAS_clumped_suggestive_loci.clumped", header = T)
...
write.csv(gwasTop, "assocRes/huvevRandomAEIGWAS_gwasTop.csv", row.names = F )
...
write.csv(annotGWAS2, "resFiles/huvevRandomAEIGWAS_mappedGenes.csv", row.names = F)
write.table(data.frame(unique(annotGWAS2$comGeneID)), "resFiles/huvevRandomAEI_nearestGenesList.txt",
            row.names = F, col.names = F, quote = F)
write.table(data.frame(unique(annotGWAS2$comTxID)), "resFiles/huvevRandomAEI_nearestTranscriptList.txt",
            row.names = F, col.names = F, quote = F)

############################################################################
annotGWAS2 <- read.csv("resFiles/huvevRandomAEIGWAS_mappedGenes.csv")
...
png("assocRes/manPlots/huvevRandomAEI_manhattanPlot.png", width=1200, height=600)
